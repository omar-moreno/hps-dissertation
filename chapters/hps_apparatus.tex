...
closest tolerable distance was 15 mrad, putting the edge of layer 1 
...
value of the shaper output.  In multi-peak mode, three consecutive samples are
...
listed on Table \ref{tab:apv_specs}.  At nominal, the shaping time is set
...
The APV25 chips used to readout the sensors are mounted on a FR4 hybrid 
electronic board
...
on either side by aluminum cooling blocks. The


tensioning 
a pair of ``half-modules'' on either side by aluminum cooling blocks. The
aluminum support structure holding the cooling blocks holds the 
...
Two different sensor module designs were used for the SVT.



The first three 
layers reused the module design used for the HPS test run SVT.  Layers 4-6 
...
of the u-channels supporting 3 modules as shown on Figure \ref{fig:;46_hm}.  
